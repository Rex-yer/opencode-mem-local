...
function getTransformersWebBuildUrl(): URL {
  // Force the web/WASM build to avoid loading onnxruntime-node native module,
  // which causes Bun process exit crashes (NAPI cleanup C++ exception).
  // The web build stubs out onnxruntime-node and uses WASM backend instead.
  return new URL(
...
async function ensureTransformersLoaded(): Promise<NonNullable<typeof _transformers>> {
  ...
  // Inject onnxruntime-web into globalThis BEFORE importing transformers.web.js,
  // so the web build picks it up instead of the empty onnxruntime-node stub.
  // Without this, IS_NODE_ENV=true causes the web build to use an empty ONNX object.
  const ort = await import("onnxruntime-web");
  (globalThis as any)[Symbol.for("onnxruntime")] = ort;
  const webBuildUrl = getTransformersWebBuildUrl();
  const mod = (await import(webBuildUrl.href)) as HfTransformers;
  mod.env.allowLocalModels = true;
  mod.env.allowRemoteModels = CONFIG.embeddingAllowRemoteModels;
  mod.env.cacheDir = join(CONFIG.storagePath, ".cache");
  mod.env.localModelPath = join(CONFIG.storagePath, ".cache");
  // Keep ONNX WASM single-threaded for Bun/Node runtimes without SharedArrayBuffer.
  try {
    const backends = (mod.env as any).backends?.onnx;
    if (backends?.wasm) {
      backends.wasm.numThreads = 1;
    }
  } catch (e) {
